...
@app.command("send-sticker")
def send_sticker(
    sticker_id: int = typer.Argument(..., help="Sticker ID (1-24)"),
    chat_id: str = typer.Option(None, "--chat-id", "-c", help="Telegram chat ID (defaults to first enabled session)"),
):
    ...
    if not chat_id:
        ...
        sessions_file = config.workspace_path / "sessions.json"
