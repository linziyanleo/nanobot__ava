...
def _make_provider(config: Config):
    ...
    # Collect cross-provider configs for mini/vision/voice/image_gen models
    ...
    for alt_model in (
        ...
        config.agents.defaults.image_gen_model,
    ):
        if not alt_model:
            continue
...
def gateway(
    ...
    agent = AgentLoop(
        ...
        restrict_config_file=config.tools.restrict_config_file,
    ...
    async def _cli_progress(content: str, *, tool_hint: bool = False, is_thinking: bool = False) -> None:
        if is_thinking:
            with _thinking.pause() if _thinking else nullcontext():
                console.print(
                    f"  [dim italic]💭 {content[:120]}{'...' if len(content) > 120 else ''}[/dim italic]"
                )
            return
