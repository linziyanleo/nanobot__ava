...
export function SessionSidebar({
  ...
  const confirmRename = (filename: string) => {
    if (editValue.trim() && onRenameSession) {
      onRenameSession(filename, editValue.trim())
    }
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-0.5">
        {sessions.map((s) => (
          ...
          >
              ...
              {editingFilename === s.filename ? (
                <div className="flex items-center gap-1 min-w-0" onClick={(e) => e.stopPropagation()}>
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') confirmRename(s.filename)
                  ...
                  />
                  <button onClick={() => confirmRename(s.filename)} className="p-0.5 text-green-500 hover:text-green-400">
            ...
            {editingFilename !== s.filename && confirmDelete !== s.filename && (
            ...
            {confirmDelete === s.filename && (
              <div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                ...
                <button
                  onClick={() => { onDeleteSession(s.filename); setConfirmDelete(null) }}
