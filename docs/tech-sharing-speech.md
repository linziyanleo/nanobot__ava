...
这不是我自己编的——第三方安全报告显示，OpenClaw 3000 多个插件中，大约 10.8% 包含恶意代码。用户账户被盗刷、文件被一键清空的事情真实发生过。其实OpenClaw最开始的几次出圈也是安全问题导致的。
...
大家可能注意到 SpecAnchor 出现在了 2 个子系统里。简单说一下它的定位：**SpecAnchor 是面向 AI 开发的 spec 治理层**。它不管你怎么写 spec，它管的是——项目里的 spec 变多以后，放在哪、该加载哪份、是否过期。在 Ava 的 Harness 里，它同时承担了 State（让 AI 获取最新的模块上下文）和 Scope（通过模块契约约束 AI 的改动范围）的角色。后面讲故事的时候会再提到它。
...
- 配置 Schema 扩展（a_schema_patch / b_config_patch）
- 首次启动引导（c_onboard_patch）
- 消息总线监听（bus_patch）
- 平台通道扩展（channel_patch）
- 前端 Console 启动注入（console_patch）
- 上下文压缩 + 记忆注入（context_patch）
- 核心循环注入——数据库、Token 统计、后台任务、生命周期管理（loop_patch）
- 模型前缀兼容（provider_prefix_patch）
- 技能发现（skills_patch）
- SQLite 持久化（storage_patch）
- 模板同步（templates_patch）
- 9 个自定义工具注册（tools_patch）
- 语音转文字代理（transcription_patch）
...
### 番外：SpecAnchor——从故事里长出来的 Skill

三个故事讲完，我想单独介绍一个从这些经历里长出来的东西——**SpecAnchor**。

它不是一开始就规划好的。是在踩了"AI 改错地方"的坑之后，我意识到一件事：

> **光靠 CLAUDE.md 写"禁止改 nanobot/"是不够的。AI 需要的是具体到模块粒度的契约，而不是一条笼统的禁令。**

于是我开始在每个模块目录下放一个 `SPEC.md`，写清楚：这个模块能改什么、不能改什么、文件边界在哪、依赖谁。但新问题来了——Spec 多了之后，怎么管？哪份是最新的？AI 该加载哪份？

这就是 SpecAnchor 要解决的问题。

`[📊 slide：SpecAnchor 三级 Spec 体系信息图]`

SpecAnchor 把 Spec 组织成三级：

- **Global Spec**：项目级约束，对所有 AI 任务生效（AGENTS.md / CLAUDE.md）
- **Module Spec**：模块级契约，AI 动手前先加载，知道边界在哪
- **Task Spec**：单次任务上下文，完成后经验回流到 Module Spec

在 Harness 里，它同时承担了两个角色：
- **State 子系统**：让 AI 始终拿到最新的模块上下文，而不是靠一个巨大的 prompt 把所有信息一股脑塞进去
- **Scope 子系统**：通过模块契约，约束 AI 的改动范围

目前 SpecAnchor 作为一个 Nanobot Skill 运行，AI 在开始每个编码任务前会自动加载对应模块的 Spec。它还在完善中——**Task Spec 的知识回流机制**（把本次任务学到的教训自动更新到 Module Spec）是下一步要做的事。

一句话总结：**上下文的质量决定了 AI 决策的质量。SpecAnchor 就是用来保证上下文质量的。**
...
```
a_schema_patch → b_config_patch（互斥：a 成功则 b 跳过）→ c_onboard_patch
       ↓
  bus_patch → channel_patch → console_patch → context_patch
       ↓
  loop_patch（核心枢纽：注入数据库、Token统计、后台任务、生命周期管理）
       ↓
  provider_prefix_patch → skills_patch → storage_patch →
  templates_patch → tools_patch → transcription_patch
```
...
| provider_prefix_patch | LOW | 模型前缀兼容，逻辑简单 |
| c_onboard_patch | LOW | 首次启动引导，独立模块 |
...
- [ ] 代码行数统计对比（nanobot/ ~23K vs ava/ ~16K）
- [ ] "半闭环"示意图（已通 / 待通）
