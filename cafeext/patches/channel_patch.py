...
def apply_channel_patch() -> str:
    ...
    original_send_message = TelegramChannel._send_message
    ...
    async def batched_send(
        ...
        """Batch callback that sends merged messages via original send_message."""
        ...
        await TelegramChannel._original_send_message(None, msg)

    def patched_send_message(self: TelegramChannel, msg: Any) -> None:
        """Intercept send_message to add batching logic."""
        nonlocal batcher
        
        if batcher is None:
            batcher = MessageBatcher(
                timeout_s=1.0,
                flush_callback=batched_send,
            )
    ...
    TelegramChannel._original_send_message = original_send_message
    TelegramChannel._send_message = patched_send_message
    SessionManager._load = patched_load

    return "TelegramChannel patched with batching, SessionManager patched with backfill"
