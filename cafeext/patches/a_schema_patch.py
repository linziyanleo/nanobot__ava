"""Replace nanobot.config.schema with cafeext fork before any other import.
...
def apply_schema_patch() -> str:
    ...
    if getattr(sys.modules.get("nanobot.config.schema"), "_cafeext_fork", False):
    ...
    try:
        ...
        fork_mod._cafeext_fork = True  # marker
        ...
        logger.info("Replaced nanobot.config.schema with cafeext fork")
        return "nanobot.config.schema replaced with cafeext fork (ConsoleConfig, multi-model, channel configs)"
