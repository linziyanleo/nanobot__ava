...
specanchor:
  ...
  created: "2026-04-08"
  status: "in_progress"
  last_change: "按最新优先级重排实施目标：先验证 patch/tools/skills merge 后可继续运行，再考虑收窄/删除与后续 merge 成本"
  related_modules:
...
- [ ] 当前分支已有未提交改动是否命中本轮 merge 路径并阻塞真正执行
- [ ] merge 后 `patch / tools / skills` 的最小运行闭环是否完整
- [ ] 在运行闭环确认后，哪些 patch / skills 可以立即收窄或删除，哪些只应先保留并补回归

## 1. Requirements (Context)

- **Goal**: 将当前 sidecar 分支与最新 `upstream/main` 对齐，并优先确认 `patch / custom tools / skills` 在 merge 后仍能继续正常运行；在此基础上再收口 shared file 与长期 patch 热区冲突。
- **In-Scope**:
...
- 事实与约束:
  ...
  - 行为热区分层：
    - Batch A：`context` / `loader` / `TOOLS.md` + `schema/onboard`
...
- `ava/patches/context_patch.py`
  - 确认 build_messages 与 provider sanitize 的边界没有被 upstream 吃掉或重复
- `ava/patches/loop_patch.py`
  - 复核 upstream `AgentLoop` 变化是否打断现有 wrapper 契约
...
- [ ] 1. fetch 最新 `upstream/main`，重算 merge-base / 左右分叉，并确认当前 worktree 是否有真实阻塞
- [ ] 2. 执行 merge，并优先处理 3 个 shared files：`context.py`、`loader.py`、`TOOLS.md`
- [ ] 3. 先做运行闭环验证：确认 merge 后 `patch / custom tools / skills` 仍能注册、加载、执行基础路径
- [ ] 4. 完成 Batch A：`schema/onboard` 与 `ava/templates/TOOLS.md` 的真实运行时对齐
- [ ] 5. 完成 Batch B：`loop/context/tools/templates/console/skills` 热区复核与最小必要修补
- [ ] 6. 复核 Batch C：provider/transcription/channel 增益区，只修被 merge 打断的契约，不做额外扩改
- [ ] 7. 在运行闭环稳定后，再更新 `ava/UPSTREAM_VERSION`、`.specanchor/patch_map.md`、`.specanchor/TODO.md` 与必要 module spec
- [ ] 8. 跑最小必要回归：
  ...
  - focused runtime/config/provider: config_migration / context_prompt_cache / loop_save_turn / commands / gateway_control / provider_retry / openai_responses
- [ ] 9. 基于回归结果，再决定本轮只保留运行修复，还是顺手做低风险的收窄 / 删除
- [ ] 10. 执行 `git diff --check`，确认无格式性破坏

## 5. Execute Log

- [ ] Step 1: fetch 最新 `upstream/main` 并确认 merge-base / worktree 状态
- [ ] Step 2: merge upstream 并记录命中的 shared files / patch 热区
- [ ] Step 3: reconcile `context.py` / `loader.py` / `TOOLS.md`
- [ ] Step 4: 验证 `patch / custom tools / skills` 的基础运行闭环
- [ ] Step 5: 复核并修补 Batch A / B 热区
- [ ] Step 6: 跑 focused pytest / guardrails / diff-check
- [ ] Step 7: 更新 `UPSTREAM_VERSION` 与 `.specanchor` 治理工件
...
| `context_patch` | 上游已吸收部分 message merge / prompt template 演进 | summarize / compress / memory / digest / provider-side sanitize | `收窄` | 以上游 `build_messages` 为主，只保留 sidecar 独有清洗与注入；避免双层重复 sanitize |
| `loop_patch` | 上游 `AgentLoop` 仍是持续热区，但未覆盖 sidecar runtime spine | db / token / bg_tasks / lifecycle 注入 | `保留` | 这是 sidecar 主承重点，不是 duplicate；长期可考虑把通用 hook seam 上推 upstream |
...
| `memory` | 与 upstream memory / Dream 同域，但上游还没有同名 tool | `保留` | 短期仍需；若上游未来出现 first-party memory tool，再评估 rename 或收窄 |
...
| `memory` | 与 upstream 同名，但能力模型已完全分叉：upstream 是 Dream 文件体系，sidecar 是 categorized memory tool | `保留` | 这是实质性 fork，不是文案漂移 |
...
3. 只有在运行闭环稳定后，才处理 `github` / `summarize` / `weather` / `skill-creator` 这类 shadow skill 的删除，或 `context/channel/transcription/onboard` 的收窄。
4. `loop_patch`、`console_patch`、`tools_patch`、`storage_patch`、`page_agent` 相关能力默认视为 sidecar 主体，不以“和 upstream 同域”为由误删。
5. 真正适合独立 upstream PR 的，不是整块 sidecar patch，而是其中被证明通用且不影响 sidecar 当前运行闭环的窄切片，例如通用 bugfix、hook seam、或文档/模板同步点。
